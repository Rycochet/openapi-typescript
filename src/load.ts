...
let rootURL: URL; // root URL for load(). Only gets set once per run
...
export default async function load(schemaURL: URL, options: LoadOptions): Promise<{ [url: string]: PartialSchema }> {
  ...
  if (!rootURL) rootURL = schemaURL; // important: set the root schema URL for retrieving remote schemas
  ...
  if (schemaURL.href === rootURL.href) {
    for (const subschemaURL of Object.keys(schemas)) {
      ...
      schemas[subschemaURL] = JSON.parse(JSON.stringify(schemas[subschemaURL]), (k, v) => {
        ...
        if (url && new URL(url).href !== rootURL.href) {
          const relativeURL =
            isFile(new URL(url)) && isFile(rootURL) ? path.posix.relative(path.posix.dirname(rootURL.href), url) : url;
          return `external["${relativeURL}"]["${parts.join('"]["')}"]`; // export external ref
        }
        // scenario 2: treat all $refs in external schemas as external
        if (!url && subschemaURL !== rootURL.href) {
          const relativeURL =
            isFile(new URL(subschemaURL)) && isFile(rootURL)
              ? path.posix.relative(path.posix.dirname(rootURL.href), subschemaURL)
              : subschemaURL;
          return `external["${relativeURL}"]["${parts.join('"]["')}"]`; // export external ref
        }
